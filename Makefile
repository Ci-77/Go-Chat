...
.PHONY: build
build:generate
	go build -o ./bin/gim ./cmd/gim

.PHONY: build-all
build-all:generate lint
	# 构建 windows
	CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -o ./bin/gim ./cmd/gim
	cp ./config.example.yaml ./build/windows/config.yaml

	# 构建 linux
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o ./bin/gim ./cmd/gim
	cp ./config.example.yaml ./build/linux/config.yaml

	# 构建 mac amd
	CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -o ./bin/gim ./cmd/gim
	cp ./config.example.yaml ./build/mac/config.yaml

	# 构建 mac m1
	CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -o ./bin/gim ./cmd/gim
